...
def blackbox_function(w, x):
    ...
    noise = np.random.normal(0, 1)  # 添加噪声
    return np.sum((w - x) ** 2) + noise

# Kiefer-Wolfowitz算法，用于优化带协变量的目标函数


# def sgd_with_lr_decay(f, w_init, x, T ,a_0=0.1, c_0=0.1, gamma=0.10, delta=0.50):
...
def sgd_with_lr_decay(f, w_init, x, T, a_0=1, c_0=0.2, gamma=0.10, delta=0.50):
    ...
    for t in range(1, T + 1):
        ...
        grad_mean = np.zeros(dim)  # 初始化梯度均值为0
...
#     bias = np.mean(mean_predicted - true_values, axis=0)
#     bias_squared = np.mean(bias**2)  
    
    
#     variance = np.mean(np.mean((predicted_values - mean_predicted)**2, axis=0))
    

#     mse = np.mean(np.mean((predicted_values - true_values)**2, axis=0))
...
if __name__ == '__main__':
    ...
    for n in n_values:
        ...
        K_phi_inv = compute_inverse_kernel_matrix(covariates_points, lambda_param=1e-4)
        ...
        for _ in range(replication):
            ...
            print(test_points,np.linalg.norm(covariates_points - theta_estimates))
