...
def adversarial_learning(generator, discriminator, train_data, covariates_points, theta_estimates, K_phi_inv,K_phi_theta_inv, num_iterations, eta_G, eta_D, lambda_theta=0.99, alpha=0.01, weight_decay=1e-4):
    ...
    for t in range(num_iterations):
        for x in train_data:
            ...
            loss_theta = torch.mean((updated_theta - theta_2) ** 2)
...
if __name__ == '__main__':
    ...
    # plt.xlabel('Number of covariate points (n)')
    # plt.ylabel('MSE')
    # plt.title('MSE comparison between KRR and GAN on Test Set')
    # plt.grid(True)
    # plt.legend(title="Method")
    # plt.show()
