...
# def objective_function_prediction(theta_estimates, theta_prime, K_phi_theta_inv, f):
#     sigma=1.0
...
#     k_phi_theta = torch.tensor(gaussian_kernel_matrix(theta_prime.detach().numpy(), theta_estimates.detach().numpy(), sigma), dtype=torch.float32).flatten()
...
#     grad_f_theta_x = torch.mm(k_phi_prime, torch.mm(K_phi_theta_inv, f.unsqueeze(1)))

#     return f_theta_x, grad_f_theta_x
...
def adversarial_learning(generator, discriminator, train_data, covariates_points, theta_estimates, K_phi_inv,K_phi_theta_inv, num_iterations, eta_G, eta_D, lambda_theta=0.5, alpha=0.5, weight_decay=1e-4):
    ...
    for t in range(num_iterations):
        for x in train_data:
            ...
            f_theta_1 = objective_function_prediction(theta_estimates, theta_1,  K_phi_theta_inv,f) 
