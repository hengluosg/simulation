...
# def f(theta, x, num_samples):
#     np.random.seed(1)
#     xi_samples = np.random.normal(0, 1, num_samples)  # 生成随机样本
#     costs = [F(theta, x, xi) for xi in xi_samples]
#     return np.mean(costs)

# # 目标函数的梯度：通过目标函数 F(theta, x, xi) 的导数计算
# def grad_f(theta, x, num_samples):
#     np.random.seed(1)
#     xi_samples = np.random.normal(0, 1, num_samples)  # 生成随机样本
...
# # 随机梯度下降 (SGD) 算法
# def sgd(theta_0, x, eta_0, T, num_samples):
#     theta = theta_0
#     theta_values = []
#     for t in range(1, T + 1):
#         # 计算当前梯度
...
# # 离线阶段实现
# def offline_stage(n, T, eta_0, covariate_dim):
#     #np.random.seed(1)
#     covariates_points = grid_sample(covariate_dim, lowbound, upbound, n)
#     #covariates_points = np.random.uniform(low = lowbound , high=upbound, size=(n, covariate_dim))
#     #print(n, covariates_points.shape)
...
#         #theta_values = sgd_with_lr_decay( theta_init, x_i, eta_0, gamma, T)
#         theta_values = sgd(theta_init, x_i, eta_0, T, num_samples)
...
#         # 离线阶段
#         covariates_points, theta_estimates = offline_stage(n, T, eta_0, covariate_dim)
